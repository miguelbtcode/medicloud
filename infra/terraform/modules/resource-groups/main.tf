...
resource "azurerm_resource_group" "primary" {
  ...
  tags     = merge(var.primary_tags, { Workload = each.value })
}
...
resource "azurerm_resource_group" "dr" {
  ...
  tags     = merge(var.dr_tags, { Workload = each.value })
}
...
resource "azurerm_resource_group" "shared" {
  ...
  location = var.primary_location
  tags     = merge(var.global_tags, { Workload = "shared" })
}

resource "azurerm_resource_group" "governance" {
  name     = "rg-${var.project}-governance-${var.environment}"
  location = var.primary_location
  tags     = merge(var.global_tags, { Workload = "governance" })
}
